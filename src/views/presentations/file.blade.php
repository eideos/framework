...
<div class="form-group col-md-{{$cols or "4"}}">
    ...
    <input type="file" id="{{$name or "avatar"}}File" name="{{$name or "avatar"}}_file{{$multiple?"[]":""}}"
           ...
           {{$multiple?"multiple":""}}
           />
           @if(!empty($force_delete) && $force_delete)<button id="{{$name}}_delete" type="button" tabindex="500" title="Quitar archivos seleccionados" class="btn btn-default btn-secondary fileinput-remove fileinput-remove-button fileinput-initial-delete" style="display:none"><i class="fas fa-trash-alt"></i></button>@endif
           @if($errors->has($name))
