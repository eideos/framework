...
@section($name)
<div class="form-group col-md-{{$cols ?? "4"}}">
    @if((!isset($table)||!$table)&&(!isset($list)||!$list))
    <label for="{{$name}}">{{$label}}</label>
    @endif
    <input type="hidden" id="{{$name ?? "avatar"}}" name="{{$name ?? "avatar"}}" value="{{old($name, $value??$initialvalue??'')}}" />
    <input type="file" id="{{$name ?? "avatar"}}File" name="{{$name ?? "avatar"}}_file{{$multiple?"[]":""}}"
           value="{{old($name, $value??$initialvalue??'')}}"
           data-theme="{{$theme ?? "fas"}}"
           data-default-preview-content="{{files_preview_html($value??"")}}"
           data-max-file-size="{{$maxFileSize ?? "1500"}}"
           {{$multiple?"multiple":""}}
           />
